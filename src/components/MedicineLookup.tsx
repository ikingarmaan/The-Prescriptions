...
export const MedicineLookup: React.FC = () => {
  ...
  return (
    <div id="medicine-lookup-container" className="w-full mx-auto space-y-6">
      ...
      {/* Error View */}
      {error && (
        <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 text-rose-800 dark:text-rose-200 text-sm flex items-start gap-3">
      ...
      {isLoading && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 text-center space-y-4">
          ...
          <div>
            <h3 className="font-bold text-slate-800 dark:text-slate-200 text-base">Retrieving Clinical Information...</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Analyzing pharmacological profile, uses, and precautions</p>
          </div>
        </div>
      )}
      ...
      {result && !isLoading && (() => {
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs overflow-hidden">
          {/* Header */}
          <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800 bg-linear-to-b from-blue-50/50 to-white dark:from-slate-900 dark:to-slate-950">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div className="space-y-2 flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300">
                    {result.drugClass || 'Therapeutic Agent'}
                  </span>
                  {isBrand ? (
                    <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/50 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-800 flex items-center gap-1">
                      <Building2 className="w-3 h-3 text-blue-600 dark:text-blue-400" />
                      Company Brand
                    </span>
                  ) : (
                    <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
                ...
                <div className="p-3 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-2xs space-y-2 text-xs">
                  <div className="flex flex-wrap items-center justify-between gap-1.5">
                    <div>
                      <span className="text-slate-400 dark:text-slate-500 font-medium">Active Chemical Molecule: </span>
                      <strong className="text-slate-800 dark:text-slate-100 text-sm font-semibold">{activeSalt}</strong>
                      {companyName && (
                        <span className="text-slate-500 dark:text-slate-400 ml-2 font-normal">
                          (Manufacturer: <strong className="text-slate-700 dark:text-slate-200">{companyName}</strong>)
                        </span>
                      )}
                    </div>
                    ...
                    <div className="pt-1.5 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {popularBrands.slice(0, 6).map((b, bIdx) => (
                          <span
                            key={bIdx}
                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-[11px]"
                          >
                            <span className="font-semibold text-slate-800 dark:text-slate-200">{b.brandName}</span>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs shrink-0 self-start">
                ...
                <p className="text-slate-600 dark:text-slate-300">{result.foodInstructions}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 md:p-8 space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {result.primaryUses.map((use, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700 text-xs font-medium text-slate-800 dark:text-slate-200 flex items-start gap-2"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200 mb-1.5 flex items-center gap-1.5">
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {result.precautionsAndWarnings.map((warning, idx) => (
                    <div
                      key={idx}
                      className="p-2.5 rounded-lg bg-amber-50/50 dark:bg-amber-950/30 border border-amber-200/60 dark:border-amber-900/50 text-xs text-amber-950 dark:text-amber-200 flex items-start gap-2"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-600 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800">
              {result.missedDoseAdvice && (
                <div>
                  <span className="font-bold text-slate-800 dark:text-slate-200 block mb-0.5">Missed Dose:</span>
                  <p>{result.missedDoseAdvice}</p>
                </div>
              )}
              {result.storageInstructions && (
                <div>
                  <span className="font-bold text-slate-800 dark:text-slate-200 block mb-0.5">Storage:</span>
      ...
      <section className="mt-12 pt-8 border-t border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 space-y-10 leading-relaxed">
        {/* Guide Header */}
        <header className="space-y-3 text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 text-xs font-black uppercase tracking-wider border border-blue-300 dark:border-blue-800">
        ...
        {/* Section 1 */}
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-400 flex items-center justify-center font-black shrink-0">
              1
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-blue-700 dark:text-blue-400">Pharmacological Foundations</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-xl border border-slate-200 dark:border-slate-700/80 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-slate-700 dark:text-slate-300">
              <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                <strong className="text-blue-900 dark:text-blue-300 block font-bold mb-0.5">Diclofenac Potassium vs. Sodium</strong>
              ...
              <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
                <strong className="text-blue-900 dark:text-blue-300 block font-bold mb-0.5">Amlodipine Besylate vs. Maleate</strong>
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 flex items-center justify-center font-black shrink-0">
              2
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Regulatory Standards</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-400 flex items-center justify-center font-black shrink-0">
              3
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-purple-700 dark:text-purple-400">Metric Precision</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
          ...
          <div className="overflow-x-auto my-3">
            <table className="w-full text-left text-xs border-collapse border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700 text-slate-700 dark:text-slate-300">
                <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="p-3 font-mono font-bold text-purple-900 dark:text-purple-300">Gram (g or gm)</td>
                  ...
                  <td className="p-3 text-rose-700 dark:text-rose-400 font-medium">Never confuse &quot;1g&quot; with &quot;1mg&quot;; doing so represents a 1,000x dosage differential.</td>
                </tr>
                <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="p-3 font-mono font-bold text-purple-900 dark:text-purple-300">Milligram (mg)</td>
                  ...
                  <td className="p-3 text-slate-600 dark:text-slate-400">Standard for 90% of oral solid dosage tablets and capsules.</td>
                </tr>
                <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="p-3 font-mono font-bold text-purple-900 dark:text-purple-300">Microgram (mcg or &mu;g)</td>
                  ...
                  <td className="p-3 text-rose-700 dark:text-rose-400 font-medium">Never use the abbreviation &quot;&mu;g&quot; in cursive handwritten notes; it easily looks like &quot;mg&quot;.</td>
                </tr>
                <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="p-3 font-mono font-bold text-purple-900 dark:text-purple-300">International Unit (IU or U)</td>
                  ...
                  <td className="p-3 text-rose-700 dark:text-rose-400 font-medium">Always use an official U-100 insulin syringe for insulin; never use standard household teaspoons or generic ml droppers.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 flex items-center justify-center font-black shrink-0">
              4
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-amber-700 dark:text-amber-400">Drug Delivery Technology</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              <span className="font-mono font-bold text-amber-800 dark:text-amber-300 text-xs uppercase bg-amber-100 dark:bg-amber-950/60 px-2 py-0.5 rounded-md">
                Enteric-Coated (EC)
              </span>
              <h4 className="font-bold text-slate-900 dark:text-white">Acid-Resistant Polymer Barrier</h4>
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white">Gradual Hydrophilic Matrix Diffusion</h4>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              ...
              <h4 className="font-bold text-slate-900 dark:text-white">Direct Venous Mucosal Absorption</h4>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
          ...
          <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/60 text-xs text-rose-950 dark:text-rose-200 space-y-1">
            <strong className="font-bold flex items-center gap-1.5 text-rose-900 dark:text-rose-300">
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-rose-100 dark:bg-rose-950/60 text-rose-700 dark:text-rose-400 flex items-center justify-center font-black shrink-0">
              5
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-rose-700 dark:text-rose-400">Clinical Pharmacology</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
          ...
          <div className="space-y-3 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 font-bold flex items-center justify-center shrink-0 text-xs">
                CYP
              </div>
              <div>
                <strong className="text-slate-900 dark:text-white font-bold block mb-0.5">The Grapefruit Juice Interaction (CYP3A4 Inhibition)</strong>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 font-bold flex items-center justify-center shrink-0 text-xs">
                Ca++
              </div>
              <div>
                <strong className="text-slate-900 dark:text-white font-bold block mb-0.5">Dairy Calcium &amp; Heavy Mineral Chelation (Fluoroquinolones &amp; Tetracyclines)</strong>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-start gap-3">
              <div className="w-8 h-8 rounded-lg bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 font-bold flex items-center justify-center shrink-0 text-xs">
                EtOH
              </div>
              <div>
                <strong className="text-slate-900 dark:text-white font-bold block mb-0.5">Ethanol (Alcohol) Conflicts</strong>
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-teal-100 dark:bg-teal-950/60 text-teal-700 dark:text-teal-400 flex items-center justify-center font-black shrink-0">
              6
            </div>
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-teal-700 dark:text-teal-400">Consumer Health Literacy</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
          ...
          <div className="bg-teal-50 dark:bg-teal-950/30 p-5 rounded-xl border border-teal-200 dark:border-teal-800/60 space-y-2 text-xs sm:text-sm text-teal-950 dark:text-teal-200">
        ...
        <article className="space-y-4 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-[11px] font-black uppercase tracking-wider text-blue-700 dark:text-blue-400">Storage Protocols</span>
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              <h4 className="font-bold text-slate-900 dark:text-white">Controlled Room Temperature (15&deg;C - 25&deg;C / 59&deg;F - 77&deg;F)</h4>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              <h4 className="font-bold text-slate-900 dark:text-white">Refrigerated Cold Chain (2&deg;C - 8&deg;C / 36&deg;F - 46&deg;F)</h4>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 space-y-2">
              <h4 className="font-bold text-slate-900 dark:text-white">The Bathroom Cabinet Myth: Moisture &amp; Humidity</h4>
              <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
        ...
        {/* Section 8 */}
        <article className="space-y-4 bg-slate-900 dark:bg-slate-900/90 border border-slate-800 text-white p-6 sm:p-8 rounded-2xl shadow-xl">
          <div className="flex items-center gap-3">
